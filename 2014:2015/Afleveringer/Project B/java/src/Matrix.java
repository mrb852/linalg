...
public class Matrix {
   ...
   * @throws IllegalArgumentException if cols() is different from B.rows()
   **/
  public Matrix mul(Matrix b) throws IllegalArgumentException {
    Matrix a = new Matrix(rows(), b.cols());
    if ( this.cols() != b.rows() ) {
      throw new IllegalArgumentException("cols is different from B.rows()");
    }
    
    for (int i = 1; i <= rows(); ++i)
      for (int j = 1; j <= b.cols(); ++j)
          for (int e = 1; e <= b.cols(); ++e)
              a.set(i, j, a.get(i, j) + get(i, e) * b.get(j, e));
